    ...
    for (NSDictionary *dict in dataArray) {
        ...
        for (NSString *string in columns) {
            if (string == columns.lastObject) {
                ...
                NSString *str = [NSString stringWithFormat:@"%@",  [dict objectForKey:string]];
                
                [sqlString appendFormat:@"'%@',", [str stringByReplacingOccurrencesOfString:@"\"" withString:@"\"\""]];
            }
        }
...
- (void)jsonToTableDB:(NSDictionary *)jsonDict
{
    ...
    NSArray *detilArray = @[@"chief_photo", @"create_time", @"designer_id", @"id", @"name", @"recommend", @"status", @"view_count", @"content"];
    ...
    NSArray *designerArray = @[@"id", @"account_enable", @"address", @"city", @"city_text", @"email", @"introduction", @"level", @"mobile", @"real_name", @"sex", @"user_id" ];
